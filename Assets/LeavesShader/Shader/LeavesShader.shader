...
Shader "Yagir/Leaves"
{
    Properties
    {
        ...
        _AmbientLightOption("Ambient Lightning", Range(0, 10)) = 3.4
    }
    SubShader
    {
        ...
        Pass{
            ...
            float4 frag (GeomData i) : SV_Target
            {
                ...
                float fogFactor = ComputeFogFactor(i.pos.z);
            	float4 finalColor = color * lerp(_BaseColor, _TipColor, i.uv.y) * (half4(unity_SHAr.w, unity_SHAg.w, unity_SHAb.w, 1) * _AmbientLightOption);

				half3 mixed = MixFog(half3(finalColor.x, finalColor.y, finalColor.z), fogFactor);
            	
                return float4(mixed.x, mixed.y, mixed.z, 0);
			}	
